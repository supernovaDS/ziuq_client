    ...
    return (
      <div className="col-span-1 md:col-span-8 group relative overflow-hidden rounded-xl bg-surface-container/40 border border-white/5 transition-all hover:bg-surface-container/60 shadow-2xl">
        <div className="absolute inset-0 z-0">
          <img alt="Featured Quiz" className="w-full h-full object-cover opacity-30 grayscale-50 transition-all duration-1000 group-hover:scale-105 group-hover:opacity-40 group-hover:grayscale-0" src={q.bannerUrl || 'https://via.placeholder.com/800x400'}/>
        ...
        <div className="p-8 md:p-12 relative z-10 flex flex-col justify-end h-full min-h-87.5">
          ...
          <button onClick={() => onStart(q)} className="flex items-center gap-2 text-primary font-bold text-xs tracking-[0.2em] uppercase group/btn self-start py-2 px-6 border border-primary/20 rounded-full hover:bg-primary/10 transition-colors">
    ...
    <div className="col-span-1 md:col-span-4 group rounded-xl bg-surface-container/40 backdrop-blur-sm p-6 flex flex-col justify-between transition-all hover:bg-surface-container/60 border border-white/5 hover:-translate-y-1 hover:shadow-2xl hover:shadow-primary/5">
      <div>
        <div className={`w-full ${isSide ? 'aspect-square' : 'h-48'} rounded-lg overflow-hidden mb-6 relative`}>
          <img alt="Quiz Thumbnail" className={`w-full h-full object-cover opacity-60 transition-all duration-700 ${isSide ? 'grayscale-30 group-hover:scale-110 group-hover:opacity-80 group-hover:grayscale-0' : 'group-hover:opacity-80 group-hover:scale-105'}`} src={q.bannerUrl || 'https://via.placeholder.com/400x400'}/>
      ...
      <div>
        {isSide ? (
           <button onClick={() => onStart(q)} className="w-full py-4 rounded-xl shadow-[0_0_15px_rgba(216,180,254,0.1)] border border-primary/30 text-primary font-bold text-xs tracking-[0.15em] uppercase hover:bg-primary hover:text-on-primary transition-all duration-300">
              Commence Sequence
           </button>
        ) : (
